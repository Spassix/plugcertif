...
import { PlugModel, Plug } from '@/lib/models/Plug'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const updatePromises = plugs.map((plug: Plug) => {
    ...
    const updatedPlugs = (await Promise.all(updatePromises)).filter((p: Plug | null): p is Plug => p !== null)
    
    return NextResponse.json({
      success: true,
      message: `Ajouté ${likesToAdd} likes à ${updatedPlugs.length} plugs`,
      plugs: updatedPlugs.map((plug: Plug) => ({
        id: plug._id,
        name: plug.name,
        likes: plug.likes
      }))
